...
def get_team():
    ...
    with sqlite3.connect("pokemon.db") as conn:
        ...
        cursor.execture(sql, (user_pk,))
        my_team = cursor.fetchall()
    return jsonify({"team": my_team})
...
def remove_pokemon():
    ...
    with sqlite3.connect("pokemon.db") as conn:
        cursor = conn.cursor()
        sql = """ DELETE from teams WHERE user_pk=? AND number=?"""
        cursor.execute(sql, (user_pk, data['pokeId']))
